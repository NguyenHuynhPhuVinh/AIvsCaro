...
class GameService {
   ...
   * Tạo game AI vs AI
   */
  async createAIvsAIGame(request: CreateGameRequest): Promise<GameResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/game/create-ai-vs-ai`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        return {
          success: false,
          message: data.error || "Lỗi khi tạo AI vs AI game",
        };
      }
