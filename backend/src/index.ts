...
fastify.post('/api/game/:gameId/join', async (request, reply) => {
  const { gameId } = request.params as { gameId: string };
  const { playerId, playerName } = request.body as { playerId: string; playerName: string };
...
fastify.post('/api/game/:gameId/move', async (request, reply) => {
  const { gameId } = request.params as { gameId: string };
  const { playerId, row, col } = request.body as { playerId: string; row: number; col: number };
  ...
  if (result.success && result.gameContext) {
    ...
    if (result.gameContext.currentPlayer === 2 && result.gameContext.gameStatus === 'playing') {
      // Tìm AI player
      const game = gameService.getGame(gameId);
      const aiPlayer = game?.players.find(p => p.isAI);
      if (aiPlayer) {
        socketService.sendToClient(aiPlayer.id, 'your_turn', result.gameContext);
      }
    }
